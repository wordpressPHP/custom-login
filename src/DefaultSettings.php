...
use PassyCo\CustomLogin\Api\FieldTypes;
use PassyCo\CustomLogin\Api\SettingsApi;
use PassyCo\CustomLogin\Customize\AlphaColorControl;
use PassyCo\CustomLogin\Customize\TextareaControl;
 ...
 */
class DefaultSettings implements WpHooksInterface {
    ...
    public function addHooks() {
        ...
        $this->addAction(
            'pre_update_option_' . SettingsApi::SETTING_ID,
            [ $this, 'combineSettingsArray' ],
            10,
            2
        );
        $this->addAction(
            'update_option_' . SettingsApi::SETTING_ID,
    ...
    protected function adminInit() {
        ...
        if ( empty( $pagenow ) || ! in_array( $pagenow, [
                'options-general.php',
                'options.php',
            ], true )
        ) {
        ...
        if ( get_option( SettingsApi::SETTING_ID ) === false ) {
            add_option( SettingsApi::SETTING_ID, [] );
        }

        // Register the settings sections.
        foreach ( $this->settings_api->getSettingsSections() as $section ) {
            add_settings_section(
                SettingsApi::SETTING_ID . '_' . $section['id'],
                $section['title'],
                '__return_false',
                SettingsApi::SETTING_ID . '_' . $section['id']
            );
        ...
        foreach ( $this->settings_api->getSettingsFields() as $section_id => $field_args ) {
            foreach ( $field_args as $option ) {
                ...
                $args = [
                    ...
                    'name' => $option['label'],
                    'section' => SettingsApi::SETTING_ID . '[' . $section_id . ']',
                    ...
                    'callback' => isset( $option['class'] ) ?
                        $option['class'] : ( new FieldTypes( $this->settings_api ) ),
                ];

                $args = wp_parse_args( $args, $option );

                add_settings_field(
                    SettingsApi::SETTING_ID . '[' . $section_id . '][' . $name . ']',
                    $option['label'],
                    [ $args['callback'], $type ],
                    SettingsApi::SETTING_ID . '_' . $section_id,
                    SettingsApi::SETTING_ID . '_' . $section_id,
                    $args
                );
            }
    ...
    public function sanitizeOptions( $options ) {
        ...
        do_action( SettingsApi::SETTING_ID . '_before_sanitize_options', $options );
        ...
        do_action( SettingsApi::SETTING_ID . '_after_sanitize_options', $options );
     ...
     */
    protected function getSanitizeCallback( $slug = '' ) {
        if ( empty( $slug ) ) {
            return false;
        }

        // Iterate over registered fields and see if we can find proper callback
        foreach ( $this->settings_api->getSettingsFields() as $section => $options ) {
     ...
     * @return array
     */
    public function combineSettingsArray( $value, $old_value ) {
    ...
    public function deleteTransients() {
        delete_transient( Common::getTransientKey( 'style' ) );
        delete_transient( Common::getTransientKey( 'script' ) );
    }

    /**
     * @return array
     */
    public static function getRegisteredSettingsSections() {
        $sections = [
            [
                'id' => 'general',
            ...
            [
                ...
                'href' => esc_url_raw( add_query_arg(
                    [
                        ...
                        [
                            'autofocus' => [ 'panel' => SettingsApi::SETTING_ID ],
                        ],
                    ],
     ...
     * @return array
     */
    public static function getRegisteredSettingsFields() {
        ...
        $fields['general'] = [
            ...
            [
                ...
                'options' => Common::getEditableRoles(),
            ],
            /** Section Login */
            [
        ...
        $fields['design'] = [
            ...
            [
                ...
                'customize' => [
                    ...
                    'add_setting' => [],
                    'add_control' => [
                        'callback' => AlphaColorControl::class,
                        'args' => [
                            'type' => 'alphacolor',
                            'palette' => true,
                        ],
            ...
            [
                ...
                'customize' => [
                    ...
                    'add_setting' => [],
                    'add_control' => [
                        'callback' => TextareaControl::class,
                        'args' => [],
                    ],
                ],
            ],
