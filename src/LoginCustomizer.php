 ...
 */
class LoginCustomizer implements WpHooksInterface {
    ...
    protected static function get_setting_id( $setting_id ) {
        return sprintf( '%s[%s][%s]', SettingsApi::SETTING_ID, self::$setting_id, $setting_id );
    }
    ...
    protected static function get_control_id( $control_id ) {
        return sprintf( '%s_%s_%s', SettingsApi::SETTING_ID, self::$setting_id, $control_id );
    }
    ...
    protected static function get_wp_customize_section_array( array $args, $title = '' ) {
        $defaults = [
            'title' => $title,
            'panel' => SettingsApi::SETTING_ID, // WordPress 4.3
            'capability' => Common::getOption( 'capability', 'general', 'manage_options' ),
            'priority' => 10,
            'description' => '',
    ...
    public function register( \WP_Customize_Manager $wp_customize ) {
        $wp_customize->add_panel( SettingsApi::SETTING_ID,
            [
                'priority' => 10,
                'capability' => Common::getOption( 'capability', 'general', 'manage_options' ),
                'title' => __( 'Custom Login', CustomLogin::DOMAIN ),
                'description' => wpautop( sprintf( __( 'Style your wp-login.php page with ease.%s', CustomLogin::DOMAIN ), '&trade;' ) ),
                'active_callback' => [ 'Common', 'is_wp_login_php' ],
            ]
        );

        $default_settings = DefaultSettings::getRegisteredSettingsFields();
        $design_settings  = ! empty( $default_settings['design'] ) ? $default_settings['design'] :
            [];
        ...
        foreach ( $design_settings as $key => $setting ) {
            ...
            $setting_name = isset( $setting['name'] ) ? $setting['name'] :
                sanitize_key( $setting['label'] );
    ...
    public function login_head() {
        if ( is_customize_preview() ) {
            Common::renderView( 'login/wp-login' );
        }
    }
    ...
    public function enqueue_script() {
        wp_enqueue_script(
            CustomLogin::DOMAIN . '-customizer',
            plugins_url( 'js/customizer.js', CUSTOM_LOGIN_FILE ),
            [ 'jquery', 'jquery-ui-core', 'jquery-ui-slider' ],
    ...
    public static function generate_css( $selector, $style, $mod_name, $prefix = '', $postfix = '', $echo = true ) {
        $return = '';
        $css    = Common::getOption( $mod_name, 'design', '' );
