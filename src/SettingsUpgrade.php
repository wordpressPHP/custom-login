...
class SettingsUpgrade implements WpHooksInterface {

    public function addHooks() {
        add_action( 'admin_notices', [ $this, 'upgrade_notices' ] );
        add_action( 'admin_menu', [ $this, 'add_submenu_page' ] );
        add_action( 'wp_ajax_custom_login_trigger_upgrades', [ $this, 'trigger_upgrades' ] );
    }
    ...
    public function upgrade_notices() {
        ...
        if ( false !== ( $old_settings = get_option( 'custom_login_settings', false ) ) ) {
            ...
            if ( version_compare( $cl_version, '2.0', '<' ) ) {
                printf(
                    ...
                    esc_url( add_query_arg( [ 'ver' => '2.0' ], admin_url( 'options.php?page=custom-login-upgrades' ) ) )
                );
            }
        } // 2.0
        ...
        if ( false !== ( $old_settings = get_option( 'custom_login', false ) ) ) {
            ...
            if ( version_compare( $cl_version, '3.0', '<' ) ) {
                printf(
                    ...
                    add_query_arg( [ 'ver' => '3.0' ], admin_url( 'options.php?page=custom-login-upgrades' ) )
                );
            }
        } // 3.0
        ...
        if ( version_compare( CUSTOM_LOGIN_VERSION, '3.1', '<' ) ) {
            printf(
                ...
                esc_url( add_query_arg( [ 'ver' => '3.1' ], admin_url( 'options.php?page=custom-login-upgrades' ) ) )
            );
        } // 3.1
    }
    ...
    private function cl_v20_upgrades() {
        ...
        $new_settings = get_option( 'custom_login', [] );

        $new_settings['version']                              = $this->parent->version;
        $new_settings['active']                               = true === $old_settings['custom'] ?
            'on' :
            'off';
        $new_settings['html_background_color']                = ScriptsStyles::is_rgba( $old_settings['html_background_color'] ) ?
            ScriptsStyles::rgba2hex( $old_settings['html_background_color'] ) :
        ...
        $new_settings['login_form_background_color']          = ScriptsStyles::is_rgba( $old_settings['html_background_color'] ) ?
            ScriptsStyles::rgba2hex( $old_settings['login_form_background_color'] ) :
            $old_settings['login_form_background_color'];
        ...
        $new_settings['login_form_border_color']              = ScriptsStyles::is_rgba( $old_settings['html_background_color'] ) ?
            ScriptsStyles::rgba2hex( $old_settings['login_form_border_color'] ) :
            $old_settings['login_form_border_color'];
        ...
        $new_settings['login_form_box_shadow_color']          = ScriptsStyles::is_rgba( $old_settings['html_background_color'] ) ?
            ScriptsStyles::rgba2hex( $old_settings['login_form_box_shadow_4'] ) :
            $old_settings['login_form_box_shadow_4'];
        $new_settings['login_form_box_shadow_color_checkbox'] = 'off';
        $new_settings['login_form_box_shadow_color_opacity']  = '';
        $new_settings['label_color']                          = ScriptsStyles::is_rgba( $old_settings['html_background_color'] ) ?
            ScriptsStyles::rgba2hex( $old_settings['label_color'] ) : $old_settings['label_color'];
    ...
    private function cl_v30_upgrades() {
        $old_settings     = get_option( 'custom_login', [] );
        $design_settings  = get_option( CUSTOM_LOGIN_OPTION . '_design', [] );
        $general_settings = get_option( CUSTOM_LOGIN_OPTION . '_general', [] );
     ...
     * @since    12/26/2014
     */
    private function get_old_setting( $setting = [], $option = null, $default = '' ) {
        if ( is_null( $option ) ) {
            return $default;
        }
